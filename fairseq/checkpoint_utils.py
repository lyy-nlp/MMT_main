...
def load_checkpoint_to_cpu(path, arg_overrides=None):
    ...
    args = state["args"]
    if isinstance(args.arch, list):
        args.arch = tuple(args.arch)
...
def load_model_ensemble(filenames, arg_overrides=None, task=None):
    ...
    """
    ensemble, ensemble_nmt, args, _task = load_model_ensemble_and_task(filenames, arg_overrides, task)
...
def load_model_ensemble_and_task(filenames, arg_overrides=None, task=None):
    ...
    for filename in filenames:
        ...
        ensemble_nmt.append(model_nmt)
    return ensemble, ensemble_nmt, args, task
...
def _upgrade_state_dict(state):
    ...
    for arch_name in state["args"].arch:
        if arch_name in models.ARCH_MODEL_REGISTRY:
            registry.set_defaults(state["args"], models.ARCH_MODEL_REGISTRY[arch_name])
