...
class Trainer(object):
    ...
    def __init__(self, args, task, model, model_nmt,criterion):
        self.args = args
        self.task = task
        ...
        self._model = model
        self._model_nmt = model_nmt
        self.cuda = torch.cuda.is_available() and not args.cpu
        if args.fp16:
            self._criterion = self._criterion.half()
            self._model = self._model.half()
            self._model_nmt = self._model_nmt.half()
        if self.cuda:
            self._criterion = self._criterion.cuda()
            self._model = self._model.cuda()
            self._model_nmt = self._model_nmt.cuda()
    ...
    @property
    def model_nmt(self):
        if self._wrapped_model_nmt is None:
            if self.args.distributed_world_size > 1 and not self.args.use_bmuf:
                self._wrapped_model_nmt = models.DistributedFairseqModel(
                    self.args, self._model_nmt
                )
            else:
                self._wrapped_model_nmt = self._model_nmt
        return self._wrapped_model_nmt
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output = self.task.train_step(
                        sample, self.model, self.model_nmt,self.criterion, self.optimizer, ignore_grad
                    )
                    del loss
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.model_nmt,self.criterion
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
