...
class TransformerEncoderLayer_nmt(nn.Module):
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def mask(self, x, lay_idx):    #生成一个遮罩矩阵（mask matrix
        ...
        mask_txt = F.softmax(x, dim=-1)

        mask_matrix_output = self.getBinaryTensor(mask_txt, 0.02)
    ...
    def forward(self, x,encoder_padding_mask,batch_len, lay_idx,
        ...
        np.save('/home/lyy/Gating2-fairseq-inter2/注意力矩阵/x_nmt.npy', x_numpy)'''

        return x



class TransformerDecoderLayer_nmt(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
            ...
            incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,  #在增量解码中，模型在生成每个标记时会保持一些先前的状态信息，以便正确生成下一个标记，而不必重新计算所有的中间结果。
