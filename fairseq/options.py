...
def parse_args_and_arch(
    ...
    if hasattr(args, "arch"):
        for arch in args.arch:
            model_specific_group = parser.add_argument_group(
                f"Model-specific configuration for {arch}",
                argument_default=argparse.SUPPRESS,
            )
            ARCH_MODEL_REGISTRY[arch].add_args(model_specific_group)
    ...
    if hasattr(args, "arch"):
        for arch in args.arch:
            if arch in ARCH_CONFIG_REGISTRY:
...
def add_model_args(parser):
    ...
    group.add_argument('--arch', '-a', default=('fconv',), metavar='ARCH',
                       choices=ARCH_MODEL_REGISTRY.keys(), nargs='+',
