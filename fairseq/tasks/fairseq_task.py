...
class FairseqTask(object):
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        """
        raise NotImplementedError   #意味着这个方法的具体实现应该由子类来完成
    ...
    def train_step(self, sample, model, model_nmt,criterion, optimizer, ignore_grad=False):
        """
        ...
        """
        model.train()
        loss, sample_size, logging_output = criterion(model, model_nmt,sample)
        if ignore_grad:
            loss *= 0
        optimizer.backward(loss)
        return loss, sample_size, logging_output

    def valid_step(self, sample, model, model_nmt,criterion):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output = criterion(model, model_nmt,sample)
        return loss, sample_size, logging_output
